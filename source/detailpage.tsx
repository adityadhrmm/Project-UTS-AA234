...
type Props = {
  data?: MenuData;
  route?: { params?: { data?: MenuData; item?: MenuData } };
  navigation?: any;
  goBack?: () => void;
...
export default function DetailMenu({ data: propData, route, navigation, goBack, onAddToCart, onBuyNow }: Props) {
  // Support data passed as prop (component usage) OR via navigation params (route.params.data or route.params.item)
  const data = propData ?? route?.params?.data ?? route?.params?.item;
  // debug: helpful when testing
  if (typeof console !== "undefined") console.log("DetailPage data:", data);
  ...
  // Fallback images map by menu name (use local require images)
  const fallbackImages: { [k: string]: any } = {
    Cappuccino: require("./asset/cappucino.jpeg"),
    Latte: require("./asset/latte.jpeg"),
    Americano: require("./asset/americano.jpeg"),
    Mocha: require("./asset/mocha.jpeg"),
  };

  // Resolve image source: remote URL (string) -> {uri}, local require -> number, or fallback by name
